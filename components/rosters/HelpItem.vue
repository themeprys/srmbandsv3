    ...
    <section class="section" v-for="happyhelp in data.objects" :key="happyhelp.id">
      <nav class="breadcrumb is-centered" aria-label="breadcrumbs">
        <ul>
          ...
          <li>
            <a href="#">Happy to Help</a>
          </li>
          <li class="is-active">
            <a href="#" aria-current="page">{{ happyhelp.title }}</a>
          </li>
        </ul>
      </nav>
      <div>
        ...
        >{{ happyhelp.title }}</h1>
        <center>
          <img class="srm_image_item" :src="happyhelp.metadata.foto.url" alt="SRM Bands" />
        </center>
        <div class="columns is-mobile">
          <div class="column is-three-fifths is-offset-one-fifth srm_page_item">
            <div v-html="happyhelp.content"></div>
          </div>
        </div>        
      </div>
    </section>
...
export default {
 head() {
    return {
      title: (this.data?.objects[0].title || "Happy to Help") + " - SRM Bookings and Services Official Site",
      meta: [
  ...
  // async mounted() {
  //  let uri = "https://cms.xabi.us/api/v2/srmartist/" + this.$route.params.slug;
  //   axios.get(uri).then((response) => {
  //       console.log(response.data)
  //     this.artistItem = response.data[0];
  //   });
  // },
};
</script>
